...
var import_safe_stable_stringify = /* @__PURE__ */ __toESM(require_safe_stable_stringify());
import_safe_stable_stringify.configure;
var stringify$1 = import_safe_stable_stringify.default;
//#endregion
//#region node_modules/spacetimedb/dist/index.browser.mjs
...
function coerceParams(params) {
	return Object.fromEntries(Object.entries(params).map(([n, c]) => [n, "typeBuilder" in c ? c.typeBuilder : c]));
}
...
function evaluateData(data, row) {
	switch (data.type) {
		case "eq": return resolveValue(data.left, row) === resolveValue(data.right, row);
		case "ne": return resolveValue(data.left, row) !== resolveValue(data.right, row);
		case "gt": return resolveValue(data.left, row) > resolveValue(data.right, row);
		case "gte": return resolveValue(data.left, row) >= resolveValue(data.right, row);
		case "lt": return resolveValue(data.left, row) < resolveValue(data.right, row);
		case "lte": return resolveValue(data.left, row) <= resolveValue(data.right, row);
		case "and": return data.clauses.every((c) => evaluateData(c, row));
		case "or": return data.clauses.some((c) => evaluateData(c, row));
		case "not": return !evaluateData(data.clause, row);
	}
}
function resolveValue(expr, row) {
	if (isLiteralExpr(expr)) return toComparableValue(expr.value);
...
function isTimestampLike(value) {
	...
	return typeof value["__timestamp_micros_since_unix_epoch__"] === "bigint";
}
function toComparableValue(value) {
	if (isHexSerializableLike(value)) return value.toHexString();
	if (isTimestampLike(value)) return value.__timestamp_micros_since_unix_epoch__;
	return value;
}
...
var EventTableRows = t.object("EventTableRows", { get events() {
	return BsatnRowList;
} });
...
var QueryRows = t.object("QueryRows", { get tables() {
	return t.array(SingleTableRows);
} });
var QuerySetId = t.object("QuerySetId", { id: t.u32() });
...
var TransactionUpdate = t.object("TransactionUpdate", { get querySets() {
...
var stringify = (value) => stringify$1(value, (key, current) => {
	...
	if (current && typeof current === "object" && "__identity__" in current && typeof current.__identity__ === "bigint") return u256ToHexString(current.__identity__);
	if (current && typeof current === "object" && "__connection_id__" in current && typeof current.__connection_id__ === "bigint") return u128ToHexString(current.__connection_id__);
	if (current instanceof Uint8Array) {
		if (current.length < 25) return `0x${toHex(current)}`;
...
var TableCacheImpl = class {
	...
	*/
	...
	#makeReadonlyIndex(tableDef, idx) {
		if (idx.algorithm !== "btree") throw new Error("Only btree indexes are supported in TableCacheImpl");
		...
		const matchRange = (row, rangeArg) => {
			...
			for (let i = 0; i < prefixLen; i++) if (!deepEqual(key[i], arr[i])) return false;
		...
		const isUnique = tableDef.constraints.some((constraint) => {
			if (constraint.constraint !== "unique") return false;
			return deepEqual(constraint.columns, idx.columns);
		});
		const self = this;
		if (isUnique) return { find: (colVal) => {
			const expected = Array.isArray(colVal) ? colVal : [colVal];
			for (const row of self.iter()) if (deepEqual(getKey(row), expected)) return row;
			return null;
		} };
		else return { *filter(range) {
			for (const row of self.iter()) if (matchRange(row, range)) yield row;
		} };
	}
	/**
	...
	*/
	...
	applyOperations = (operations, ctx) => {
		...
		if (this.tableDef.isEvent) {
			for (const op of operations) if (op.type === "insert") pendingCallbacks.push({
				type: "insert",
		...
		if (this.hasPrimaryKey) {
			...
			for (const op of operations) if (op.type === "insert") {
			...
			for (const [primaryKey, [insertOp, refCount]] of insertMap) {
				...
				if (deleteEntry) {
					...
					if (maybeCb) pendingCallbacks.push(maybeCb);
		...
		} else for (const op of operations) if (op.type === "insert") {
...
function comparePreReleases(a, b) {
	...
	for (let i = 0; i < len; i++) {
		...
		if (typeof aPart === "string" && typeof bPart === "string") return aPart.localeCompare(bPart);
...
var SemanticVersion = class _SemanticVersion {
	...
	toString() {
		...
		if (this.preRelease) versionString += `-${this.preRelease.join(".")}`;
		if (this.buildInfo) versionString += `+${this.buildInfo}`;
	...
	compare(other) {
		...
		if (this.preRelease && other.preRelease) return comparePreReleases(this.preRelease, other.preRelease);
	...
	static parseVersionString(version) {
		const match = version.match(/^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-([\da-zA-Z-]+(?:\.[\da-zA-Z-]+)*))?(?:\+([\da-zA-Z-]+(?:\.[\da-zA-Z-]+)*))?$/);
		if (!match) throw new Error(`Invalid version string: ${version}`);
		return new _SemanticVersion(parseInt(match[1], 10), parseInt(match[2], 10), parseInt(match[3], 10), match[4] ? match[4].split(".").map((id) => isNaN(Number(id)) ? id : Number(id)) : null, match[5] || null);
	}
};
var _MINIMUM_CLI_VERSION = new SemanticVersion(1, 4, 0);
function ensureMinimumVersionOrThrow(versionString) {
	if (versionString === void 0) throw new Error(versionErrorMessage(versionString));
	if (SemanticVersion.parseVersionString(versionString).compare(_MINIMUM_CLI_VERSION) < 0) throw new Error(versionErrorMessage(versionString));
}
...
async function decompress(buffer, type, chunkSize = 128 * 1024) {
	let offset = 0;
	const readableStream = new ReadableStream({ pull(controller) {
...
async function openWebSocket({ url, nameOrAddress, wsProtocol, authToken, compression, lightMode, confirmedReads }) {
	...
	if (authToken) {
		...
		} else throw new Error(`Failed to verify token: ${response.statusText}`);
	}
	const databaseUrl = new URL(`v1/database/${nameOrAddress}/subscribe`, url);
	if (temporaryAuthToken) databaseUrl.searchParams.set("token", temporaryAuthToken);
	databaseUrl.searchParams.set("compression", {
		gzip: "Gzip",
		brotli: "Brotli",
		none: "None"
	}[compression] ?? "None");
	if (lightMode) databaseUrl.searchParams.set("light", "true");
	if (confirmedReads !== void 0) databaseUrl.searchParams.set("confirmed", confirmedReads.toString());
...
var WebsocketDecompressAdapter = class _WebsocketDecompressAdapter {
	...
	set onmessage(handler) {
		this.#ws.onmessage = async (msg) => {
			handler({ data: await this.#decompress(new Uint8Array(msg.data)) });
		};
	}
	set onerror(handler) {
	...
	async #decompress(buffer) {
		...
		switch (tag) {
			case 0: return data;
			case 1: return await decompress(data, "brotli");
			case 2: return await decompress(data, "gzip");
			default: throw new Error("Unexpected Compression Algorithm. Please use `gzip` or `none`");
		}
	}
	send(msg) {
		this.#ws.send(msg);
...
var DbConnectionBuilder = class {
	...
	*/
	build() {
		if (!this.#uri) throw new Error("URI is required to connect to SpacetimeDB");
		if (!this.#nameOrAddress) throw new Error("Database name or address is required to connect to SpacetimeDB");
...
var SubscriptionBuilderImpl = class {
	...
	*/
	...
	subscribe(query_sql) {
		...
		} else queries = Array.isArray(query_sql) ? query_sql : [query_sql];
		if (queries.length === 0) throw new Error("Subscriptions must have at least one query");
...
var SubscriptionHandleImpl = class {
	...
	*/
	unsubscribe() {
		if (this.#unsubscribeCalled) throw new Error("Unsubscribe has already been called");
	...
	*/
	unsubscribeThen(onEnd) {
		if (this.#endedState) throw new Error("Subscription has already ended");
		if (this.#unsubscribeCalled) throw new Error("Unsubscribe has already been called");
		this.#unsubscribeCalled = true;
		this.db.unregisterSubscription(this.#querySetId);
...
function normalizeWsProtocol(protocol) {
	if (protocol === V3_WS_PROTOCOL) return V3_WS_PROTOCOL;
	if (protocol === "" || protocol === V2_WS_PROTOCOL) return V2_WS_PROTOCOL;
...
function forEachServerMessageV3(reader, data, visit) {
	reader.reset(data);
	if (reader.remaining === 0) throw new RangeError(EMPTY_V3_PAYLOAD_ERR);
	let count = 0;
...
function getClientMessageVariantTag(name) {
	if (ClientMessage.algebraicType.tag !== "Sum") throw new TypeError("ClientMessage must be a sum type");
	const tag = ClientMessage.algebraicType.value.variants.findIndex((variant) => variant.name === name);
	if (tag === -1) throw new RangeError(`Unknown ClientMessage variant: ${name}`);
	return tag;
}
...
var DbConnectionImpl = class {
	...
	*/
	...
	ws;
	wsPromise;
	constructor({ uri, nameOrAddress, identity, token, emitter, remoteModule, createWSFn, compression, lightMode, confirmedReads }) {
		stdbLogger("info", "Connecting to SpacetimeDB WS...");
		const url = new URL(uri.toString());
		if (!/^wss?:/.test(uri.protocol)) url.protocol = url.protocol === "https:" ? "wss:" : "ws:";
	...
	#parseRowList(type, tableName, rowList) {
		...
		while (reader.remaining > 0) {
			...
			let rowId = void 0;
			if (primaryKeyColName !== void 0 && primaryKeyColType !== void 0) rowId = AlgebraicType.intoMapKey(primaryKeyColType, row[primaryKeyColName]);
			else rowId = (0, import_base64_js.fromByteArray)(buffer.subarray(previousOffset, reader.offset));
	...
	#mergeTableUpdates(updates) {
		...
		for (const update of updates) {
			...
			if (ops) for (const op of update.operations) ops.push(op);
			else merged.set(update.tableName, update.operations.slice());
		}
	...
	#queryRowsToTableUpdates(rows, opType) {
		const updates = [];
		for (const tableRows of rows.tables) updates.push({
	...
	#tableUpdateRowsToOperations(tableName, rows) {
		...
		if (rows.tag === "EventTable") return this.#parseRowList("insert", tableName, rows.value.events);
		return [];
	}
	#querySetUpdateToTableUpdates(querySetUpdate) {
		...
		for (const tableUpdate of querySetUpdate.tables) {
			let operations = [];
			for (const rows of tableUpdate.rows) operations = operations.concat(this.#tableUpdateRowsToOperations(tableUpdate.tableName, rows));
			updates.push({
	...
	#flushOutboundQueueV2(wsResolved) {
		...
		for (const message of pending) wsResolved.send(message);
	}
	#flushOutboundQueueV3(wsResolved) {
		...
		if (this.#outboundQueue.length > 0) this.#scheduleDeferredOutboundFlush();
	}
	...
	#scheduleOutboundFlushWith(schedule) {
		if (this.#isOutboundFlushScheduled) return;
		this.#isOutboundFlushScheduled = true;
		const flush = () => {
			this.#isOutboundFlushScheduled = false;
			if (this.ws && this.isActive) this.#flushOutboundQueue(this.ws);
		};
		if (schedule === "next-task") setTimeout(flush, 0);
	...
	#sendEncodedMessage(encoded, describe) {
		...
		} else this.#outboundQueue.push(encoded.slice());
	}
	#sendMessage(message) {
		...
		this.#sendEncodedMessage(encoded, () => isLive ? `Sending message to server: ${stringify(message)}` : `Queuing message to server: ${stringify(message)}`);
	}
	...
	*/
	#handleOnOpen() {
		if (this.ws) this.#negotiatedWsProtocol = normalizeWsProtocol(this.ws.protocol);
		this.isActive = true;
	...
	#applyTableUpdates(tableUpdates, eventContext) {
		...
		for (const tableUpdate of tableUpdates) {
			...
			const newCallbacks = this.clientCache.getOrCreateTable(tableDef).applyOperations(tableUpdate.operations, eventContext);
			for (const callback of newCallbacks) pendingCallbacks.push(callback);
		}
		return pendingCallbacks;
	}
	#applyTransactionUpdates(eventContext, tu) {
		...
		for (const querySetUpdate of tu.querySets) {
			...
			for (const update of tableUpdates) allUpdates.push(update);
		}
	...
	#dispatchPendingCallbacks(callbacks) {
		...
		for (const callback of callbacks) callback.cb();
	}
	#processServerMessage(serverMessage) {
		...
		switch (serverMessage.tag) {
			case "InitialConnection":
				...
				if (!this.token && serverMessage.value.token) this.token = serverMessage.value.token;
			...
			case "SubscriptionError": {
				...
				if (subscription) {
					...
					this.#subscriptionManager.subscriptions.delete(querySetId);
				} else stdbLogger("error", `Received SubscriptionError for unknown querySetId ${querySetId}:`, error);
				break;
			}
			...
			case "ProcedureResult": {
				...
				const result = status.tag === "Returned" ? {
					tag: "Ok",
					value: status.value
				} : {
					tag: "Err",
	...
	*/
	#handleOnMessage(wsMessage) {
		...
		if (this.#isDrainingInboundQueue) return;
		...
		try {
			while (this.#inboundQueueOffset < this.#inboundQueue.length) {
				...
				if (data) this.#processMessage(data);
			}
		} finally {
			if (this.#inboundQueueOffset >= this.#inboundQueue.length) this.#inboundQueue.length = 0;
			else if (this.#inboundQueueOffset > 0) this.#inboundQueue = this.#inboundQueue.slice(this.#inboundQueueOffset);
	...
	*/
	callReducer(reducerName, argsBuffer, reducerArgs) {
		const encodedReducerName = this.#reducerNameBytes[reducerName];
		if (encodedReducerName) return this.#callReducerWithEncodedName(reducerName, encodedReducerName, argsBuffer, reducerArgs);
	...
	#callReducerWithEncodedName(reducerName, encodedReducerName, argsBuffer, reducerArgs) {
		...
		if (reducerArgs) this.#reducerCallInfo.set(requestId, {
			name: reducerName,
			args: reducerArgs
		});
		this.#reducerCallbacks.set(requestId, (result) => {
			if (result.tag === "Ok" || result.tag === "OkEmpty") resolve();
			else if (result.tag === "Err") reject(new SenderError(new BinaryReader(result.value).readString()));
			else if (result.tag === "InternalError") reject(new InternalError(result.value));
			else reject(/* @__PURE__ */ new Error("Unexpected reducer result"));
		});
		return promise;
	}
	...
	*/
	callProcedure(procedureName, argsBuffer) {
		const encodedProcedureName = this.#procedureNameBytes[procedureName];
		if (encodedProcedureName) return this.#callProcedureWithEncodedName(procedureName, encodedProcedureName, argsBuffer);
	...
	#callProcedureWithEncodedName(procedureName, encodedProcedureName, argsBuffer) {
		...
		this.#procedureCallbacks.set(requestId, (result) => {
			if (result.tag === "Ok") resolve(result.value);
...
function tablesToSchema(ctx, tables) {
	...
	for (const [accName, schema2] of Object.entries(tables)) tableDefs[accName] = tableToSchema(accName, schema2, schema2.tableDef(ctx, accName));
...
function schema(tables) {
	return new Tables(tablesToSchema(new ModuleContext(), tables));
}
...
function reducersToSchema(reducers2) {
	return { reducers: reducers2.map((r) => {
...
function reducers(...args) {
	return new Reducers(args.length === 1 && Array.isArray(args[0]) ? args[0] : args);
}
function reducerSchema(name, params) {
	const paramType = { elements: Object.entries(params).map(([n, c]) => ({
		name: n,
...
function procedures(...args) {
	return { procedures: args.length === 1 && Array.isArray(args[0]) ? args[0] : args };
}
